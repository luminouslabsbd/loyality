    ...
    <section class="w-full partner-forgot-page place-self-center mx-auto lg:h-full ll-login-container">
        <div class="grid lg:grid-cols-2 lg:h-full">
            ...
            <div class="ll-login-right flex justify-center items-center py-6 px-4 lg:py-6 sm:px-0">
                <div class="w-full space-y-4 md:space-y-6 max-w-lg xl:max-w-lg p-6">
                    <h2 class="text-3xl font-bold text-gray-900 dark:text-white">{{ trans('common.registration_title') }}</h2>
                    <x-forms.messages />
                    @if (!Session::has('success'))
                        <x-forms.form-open class="ll-form-container space-y-4 md:space-y-6" :action="route('member.register.post')" method="POST" />
                        ...
                        />
                        <x-forms.checkbox 
                            class="ll-terms-policy"
                            name="consent" 
                            :label="trans('common.registration_consent', [
                                'terms_of_use' => '<a rel=\'nofollow\' tabindex=\'-1\' target=\'_blank\' class=\'text-link ll-text-primary-color font-semibold underline\' href=\''.route('member.terms').'\'>'.trans('common.terms').'</a>',
                                'privacy_policy' => '<a rel=\'nofollow\' tabindex=\'-1\' target=\'_blank\' class=\'text-link ll-text-primary-color font-semibold underline\' href=\''.route('member.privacy').'\'>'.trans('common.privacy_policy').'</a>',
                            ])" 
                        />
                    ...
                    <p class="text-sm font-light ll-text-primary-color">
                        {{ trans('common.login_text') }} <a href="{{ route('member.login') }}"
                            class="text-link font-semibold">{{ trans('common.login_link') }}</a>
                    </p>
                </div>
